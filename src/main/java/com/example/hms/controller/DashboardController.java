    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        sidePane.setOnKeyPressed(event -> {
            switch (event.getCode()) {
                case S:
                    if (controlS.match(event)) btnStudents.fire();
                    break;
                case R:
                    if (controlR.match(event)) btnRooms.fire();
                    break;
                case E:
                    if (controlE.match(event)) btnReservation.fire();
                    break;
                case L:
                    if (controlL.match(event)) btnLogout.fire();
                    break;
                default:
                    break;
            }
